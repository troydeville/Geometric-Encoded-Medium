...
// ==========================================
// FUNDAMENTAL CONSTANTS (Literals)
// ==========================================

const_def!(H, "h = 6.62607015x10⁻³⁴ J·s", 6.62607015e-34);

const_def!(E, "e = 1.602176634x10⁻¹⁹ C", 1.602176634e-19);

const_def!(C, "c = 299792458 m·s⁻¹", 299792458.0);

const_def!(PHI_LITTLE, "φ = 10^4 ((kg s)/(C m))^2", 10000.0);

const_def!(ALPHA_GAMMA, "αγ = 4580703784999263461548761·π",
    4580703784999263461548761_f64 * PI);

const_def!(ALPHA_DELTA, "αδ = 1972044687500000000000000000_f64",
    1972044687500000000000000000.0_f64);
...
// ==========================================
// DERIVED FUNCTIONS (Calculated at Runtime)
// ==========================================


/// Planck Impedance: Zp = 2h / e^2
pub fn z_p() -> f64 {
    (2.0 * H.val) / (E.val * E.val)
}

/// Vacuum Impedance: Zo = Zp * Alpha
pub fn z_0() -> f64 {
    z_p() * ALPHA.val
}

/// Geometric Scalar S = Sqrt[2] * Pi^(1/4)
pub fn gem_s() -> f64 {
    2.0_f64.sqrt() * PI.powf(0.25)
}

/// GEM Derived Gravitational Constant
/// G = Zo / (c * φ * S)
pub fn g() -> f64 {
    z_0() / (C.val * PHI_LITTLE.val * gem_s())
}

/// Vacuum Permittivity derived from Impedance
/// ε0 = 1 / (c * Zo)
pub fn epsilon_0() -> f64 {
    1.0 / (C.val * z_0())
}
...
/// Ξ = Sqrt[8π G ε0]
pub fn xi() -> f64 {
    // Note: We call the functions g() and epsilon_0() here
    (8.0 * PI * g() * epsilon_0()).sqrt()
}

/// Planck Mass derived from G
/// mP = Sqrt[(h c)/(2π G)]
pub fn m_p() -> f64 {
    ((H.val * C.val) / (2.0 * PI * g())).sqrt()
}

/// Electron Mass derived from Planck Mass
pub fn m_e() -> f64 {
    m_p() / 23_892_177_732_494_625_341_440_f64
}
