...
The engine now successfully derives $G$ from the electromagnetic properties of the vacuum ($Z_0, \phi, S$) with **99.999% accuracy** relative to CODATA observations.
...
*\*Derived via $G = \frac{Z_0}{c \phi S}$*

### Key Theoretical Confirmations
1.  **Gravity as Curvature:** Unit tests confirm `G / Go == Curvature^2`.
2.  **Horn Torus Topology:** The engine enforces a Horn Torus geometry ($R=r$). The volumetric difference between this geometry and the linear field ($1/\alpha$) accurately predicts the **Proton Radius Anomaly**.

## 🛠 Engine Updates
* **New Module:** `gravity.rs` implemented to handle emergent mass/charge interactions.
* **Refactored Constants:** Refactored `constants.rs` to use pure functions for high-precision derived constants ($G, m_P, \Xi$).
* **Visualization:** Added `gem_curvature_3d.png` visualizing the Impedance Well.

## 📄 Documentation
* Added `docs/Derivation_Gravity_From_Impedance.md`
* Added `docs/Geometric_Proof_Horn_Torus.md`
* Included Mathematica Notebooks in `derivations/mathematica/`
